# TODO:
# I think this module will be scrapped to be automatically generated by
# Haskell.
#
# I was going to write a bootstrapping compiler by hand, but it seems so much
# easier to have Haskell autogenerate one for me.  Thankfully there already
# exist available Haskell implementations.  (Even something else autogenerating
# it might be easier.)
#
# If you need to port it to a new platform, one option is simply to add a new
# front-end and cross-compile it on an old, already-supported platform.
#
# I only need to let GHC bootstrap me once, and then I can take it from there.
#
# I can add multiple frontends, e.g. one for Java, etc.
